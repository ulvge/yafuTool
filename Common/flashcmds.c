    ...
    ResLen                = sizeof(AMIYAFUAllocateMemoryRes_T);
        ...
        while (1)
        {
                ...
                if (pAMIYAFUAllocateMemoryRes.AllocmemRes.YafuCmd == CMD_AMI_YAFU_COMMON_NAK)
                {
                    ...
                    {
                        if (vdbg) { printf ("\nWarning: YAFUAllocateMemory failed %d \n", errVal);  fflush (stdout);  }
                        RetryCount += 1;
                        if (RetryCount <= RETRYCOUNT)
                        {
                            if (vdbg) { printf ("Retry #%d\n", RetryCount); fflush (stdout); }
                            continue;
                         }
                        else
                        {
                            if (vdbg) { printf ("Error: Retry Count exceeded. So trying with block by block mode\n"); fflush (stdout); }
...
#endif
{
     ...
     while (1)
    {
         ...
         if (pAMIYAFUWriteMemoryRes.WriteMemRes.YafuCmd == CMD_AMI_YAFU_COMMON_NAK )
        {
            ...
            {
                if (vdbg) { printf ("Retry #%d\n", RetryCount_NAK); fflush (stdout); }
                continue;
            }
            else
            {
                if (vdbg) { printf ("Error: Retry Count exceeded. Aborting\n"); fflush (stdout); }
                return -1;
            }
                }
            else
   ...
   ResLen                = sizeof (AMIYAFUEraseCopyFlashRes_T);
    ...
    else
    {
        ...
        if (errVal != ACCESN_OK)
        {
            ...
            {
                if (vdbg) { printf ("Retry #%d\n", RetryCount_IPMB_REQ); fflush (stdout); }
                continue;
            }
            else
            {
...
int GetBootConfig (IPMI20_SESSION_T *hSession,char *BootVariables,char *BootVar)
{
    ...
    while (1)
    {
        ...
        if(CalculateChksum((char *)&pAMIYAFUGetBootConfigRes->Status,(INT32U)pAMIYAFUGetBootConfigRes->GetBootRes.Datalen) != pAMIYAFUGetBootConfigRes->GetBootRes.CRC32chksum)
        {
            ...
            if (RetryCount_CSUM <= RETRYCOUNT)
            {
                if (vdbg) { printf ("Warning: Checksum failure GetBootConfig.  Attempting Retry #%d\n", RetryCount_CSUM);
                fflush (stdout); }
                continue;
            }
            else
            {
                if (vdbg) { printf ("Error: Checksum failure GetBootConfig.  Aborting operation\n"); fflush (stdout); }
                return -1;
            }
        }
        else
...
int SetBootConfig (IPMI20_SESSION_T *hSession,char *BootVar,char *BootVal)
{
    ...
    while (1)
    {
        ...
        if(CalculateChksum((char *)&pAMIYAFUSetBootConfigRes.Status,(INT32U)pAMIYAFUSetBootConfigRes.SetBootRes.Datalen) != pAMIYAFUSetBootConfigRes.SetBootRes.CRC32chksum)
        {
                ...
                if (RetryCount_CSUM <= RETRYCOUNT)
                {
                    if (vdbg) { printf ("Warning: Checksum failure SetBootConfig.  Attempting Retry #%d\n", RetryCount_CSUM);
                    fflush (stdout); }
                    continue;
                }
                else
                {
                    if (vdbg) { printf ("Error: Checksum failure SetBootConfig.  Aborting operation\n"); fflush (stdout); }
                    return -1;
                }
        }
        else
...
int GetAllBootVars (IPMI20_SESSION_T *hSession,unsigned char *BootVars,INT16U *BootVarsCount)
{
    ...
    while (1)
    {
        ...
        if(CalculateChksum((char *)&pAMIYAFUGetBootVarsRes->VarCount,(INT32U)pAMIYAFUGetBootVarsRes->GetBootRes.Datalen) != pAMIYAFUGetBootVarsRes->GetBootRes.CRC32chksum)
        {
            ...
            if (RetryCount_CSUM <= RETRYCOUNT)
            {
                if (vdbg) { printf ("Warning: Checksum failure GetAllBootVars.  Attempting Retry #%d\n", RetryCount_CSUM);
                fflush (stdout); }
                continue;
            }
            else
            {
                if (vdbg) { printf ("Error: Checksum failure GetAllBootVars.  Aborting operation\n"); fflush (stdout); }
                return -1;
            }
        }
        else
