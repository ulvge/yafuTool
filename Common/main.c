...
//	bIsWow64 = IsWow64();
	...
	if (*g_szExtractPath != NULL) {         // driver extracted?
...
if ( schService == NULL )
    ...
    ret = (ret1=StartService( schService, 0, NULL ))
  ...
  if(byMedium != KCS_MEDIUM)
  {
   ...
   for(DrvCount = 0;DrvCount < sizeof(OpenIPMIDriver)/sizeof(OpenIPMIDrv_T);DrvCount++)
   {
      if(OpenIPMIDriver[DrvCount].LoadModule == 1)
      {
           ...
           if((memcmp(OpenIPMIDriver[DrvCount].ModuleName,"ipmi_si",strlen(OpenIPMIDriver[DrvCount].ModuleName)) == 0) && (FlashSelected == 1)
            && (Parsing.RebootFirmware == 1))
           {
...
int PreserveUbootEnvVariables(IPMI20_SESSION_T *hSession,char *BootVarsName,INT16U* BootVarsCount)
{
    ...
    printf("Preserving Env Variables ...             \r");  fflush(stdout);
...
int SettingEnvUbootVariables(IPMI20_SESSION_T *hSession,char *BootVarsName,INT16U* BootVarsCount)
{
    ...
    memset(BootVar,0, MAX_BOOTVAR_LENGTH);
    BootVarsName += sizeof(AMIYAFUGetBootVarsRes_T);
    printf("Setting Env variables ...               \r");    fflush(stdout);
...
int FirmwareInfo (IPMI20_SESSION_T *hSession,char *imgName)
{
    ...
    {
        if (vdbg) printf("Error in retrieving FMH Headers \n");
        free(FMHRes);
        return -1;
    }
        ...
        if(i<ModuleCount)
        {
            ...
            if(spstrcasecmp((char *)FwModuleHdr[i]->ModuleName,"boot") == 0 )
            {
                ...
                BootImgRomAddr =  FwModuleHdr[i]->FmhLocation;
                          ...
                          (spstrcasecmp((char *)FwModuleHdr[i]->ModuleName,"params")) == 0 ) && (FwModConf == FALSE))
            {
        ...
        if(i < FlashFMH)
        {
            ...
            if(spstrcasecmp((char *)CurrFwModHdr[i]->ModuleName,"boot") == 0 )
            {
                ...
                BootImgFirmAddr = CurrFwModHdr[i]->FmhLocation;
                        ...
                        (spstrcasecmp((char *)CurrFwModHdr[i]->ModuleName,"params")) == 0 ) && (CurrFwModConf == FALSE))
            {
   ...
   if(SplitFlashOffset != 0)
   {
    ...
    {
        if (vdbg) printf("Error in Dislay Modules rearrange \n");
        return -1;
    }
    ...
    {    
        ...
        {
            if (vdbg) printf("Error in Dislay Modules rearrange DualFMH\n");
            return -1;
        }
    }
...
int ModLocationChkAndUpgrade (IPMI20_SESSION_T *hSession,int Module,int CurNumModule,int MaxNumModule)
{
	...
	if(RecoveryFlashMode == 0x00)
	    ...
	    {
	        oldVersion=0x01;
	        if (vdbg) printf("Older Version !Doing Firmware upgrade block by block\n");
	    }
	    else
	    {
                oldVersion=0x00;
	        if (vdbg) printf("Newer Version ! Doing Full Firmware upgrade \n");
	    }
	}
        Parsing.Full=0x01;
        while(1)
...
int NonInteractiveUpgrade(IPMI20_SESSION_T *hSession, int FlashModCnt)
{
    ...
    for ( i = 0; i < FlashModCnt; i++,k++ )
    {
        ...
        for ( j = 0; j < ModuleCount; j++)
        {
            ...
            if ((strcmp((char *)FwModuleHdr[j]->ModuleName, FlashModName[k]) == 0)
                || ((strncmp("conf", FwModuleHdr[j]->ModuleName,sizeof(FwModuleHdr[j]->ModuleName))== 0)
                     && (strncmp(FlashModName[k],"bkupconf",MAX_INPUT_LEN)==0)
                     && (confflag==2)))
            {
        ...
        for(i = 0; i < ChoiceNo; i++)
        {
            while(1)
            {
                ...
                for (j = 0,confflag = 0; j < ModuleCount; j++)
                {
                    if(strncmp("conf", FwModuleHdr[j]->ModuleName,sizeof(FwModuleHdr[j]->ModuleName)) == 0)
                        confflag++;
                    if((strncmp(ModChoice, FwModuleHdr[j]->ModuleName, sizeof(FwModuleHdr[j]->ModuleName)) == 0)
                       || ((strncmp(ModChoice,"bkupconf",MAX_INPUT_LEN)==0)
                            && (strncmp("conf", FwModuleHdr[j]->ModuleName,sizeof(FwModuleHdr[j]->ModuleName))== 0)&&(confflag==2)))
                    {
...
int main(int argc, char *argv[])
{
   ...
   if( byMedium != KCS_MEDIUM)  //Create session only for USB or nw media.
   {
        ...
        if((byMedium != USB_MEDIUM) && (Parsing.UserNameExist == 0x00)
        &&(Parsing.PasswordExist == 0x00))
        {
...
switch_device:	
    ...
    if(Parsing.ImgInfo == 0)
    {
        ...
        if(EraseBlkSize == 0) printf("Error!! The EraseBlkSize = 0.\n");
            ...
            if(!activated)
            {
                if(RecoveryFlashMode == 0x00)
                {
                ...
                else
                    {
                        Parsing.Full = 0x02;
                        oldVersion = 0x01;
                        if (vdbg) printf("Firmware is in UBOOT Mode !Doing Firmware upgrade block by block \n");
                    }
            }
            for(i=0;i<ModuleCount;i++)
            {
