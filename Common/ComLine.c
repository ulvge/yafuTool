...
static
int Parse_NonConfigurable_Arguments (int Argc, int *index, char *Arg, char *Value)
{
...
static
int Parse_Configurable_Arguments (int Argc, int *index, char *Arg, char *Value)
{
    ...
    if (spstrcasecmp(Arg, "-d") == 0)
    {
	    ...
	    if((SPIDevice != BMC_FLASH) && (SPIDevice != BIOS_FLASH ) && (SPIDevice != CPLD_FLASH )
			&& (SPIDevice != MMC_FLASH) && (SPIDevice != ME_FLASH))
	    {
		printf("%s ",Arg);
		printf(STR_ERR_OPTION_ARGUMENT);
		return -1;
    ...
    if ((spstrcasecmp(Arg, "-activate") == 0) || (spstrcasecmp(Arg, "-a") == 0))
    {
	    ...
	    if((Parsing.ActivateNode != BMC_FLASH) && (Parsing.ActivateNode != BIOS_FLASH ) 
			&& (Parsing.ActivateNode != CPLD_FLASH ))
	    {
		printf("%s ",Arg);
		printf(STR_ERR_OPTION_ARGUMENT);
		return -1;
    ...
    { 
        ...
        if ((Parsing.ImgSelect != IMAGE_1) && (Parsing.ImgSelect != IMAGE_2) 
              &&(Parsing.ImgSelect != IMAGE_BOTH) && (Parsing.ImgSelect != AUTO_INACTIVE_IMAGE)) 
        { 
            printf(" %s\n",Arg); 
            printf(STR_ERR_OPTION_ARGUMENT); 
            PrintHelp(); 
...
/*#ifdef CONFIG_SPX_FEATURE_GLOBAL_DUAL_IMAGE_SUPPORT
    ...
    if((Parsing.NwMedium == 0x00) && (Parsing.SerialMedium == 0x00) && ((Parsing.UserNameExist == 0x01)
       ||(Parsing.PasswordExist == 0x01)))
    {
    ...
    if(((Parsing.NwMedium == 0x01) || (Parsing.SerialMedium == 0x01)) && (((Parsing.UserNameExist == 0x01)
        && (Parsing.PasswordExist == 0x00)) || ((Parsing.UserNameExist == 0x00)
        && (Parsing.PasswordExist == 0x01))))
    {
    ...
    if( (Parsing.UsbMedium == 0x00) && (Parsing.NwMedium == 0x00) && (Parsing.SerialMedium == 0x00) &&(Parsing.Full == 0x00) 
    && (Parsing.Info == 0x00) && (Parsing.ImgInfo == 0x00) && (Parsing.Auto == 0x00) 
    && (Parsing.Interactive == 0x00) && (Parsing.BootPreserve == 1) && (Parsing.ConfigPreserve == 0))
    {
